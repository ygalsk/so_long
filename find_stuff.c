...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/14 17:48:41 by dkremer           #+#    #+#             */
/*   Updated: 2024/02/17 21:36:33 by dkremer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_c(t_game *game)
{
	...
	if (game->map[y][x] == COLLECTIBLE)
	{
		...
		game->player_posx = x;
		game->player_posy = y;
		mlx_image_to_window(game->mlx, game->player_images, \
								game->player_posx * 64, game->player_posy * 64);
		get_to_exit(game);
	}
	if (game->map[y][x] == EXIT)
	{
